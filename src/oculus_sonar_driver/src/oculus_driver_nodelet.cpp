...
void OculusDriver::onInit() {
  ...
  oculus_simple_ping_result_pub_ =
      n_.advertise<blueprint_oculus_msgs::OculusSimplePingResultMsg>(
          "oculus_simple_ping_result", 10);
  ...
  data_rx_.setCallback<SimplePingResultV1>(
      std::bind(&OculusDriver::pingCallback<SimplePingResultV1>, this,
                std::placeholders::_1));

  data_rx_.setCallback<SimplePingResultV2>(
      std::bind(&OculusDriver::pingCallback<SimplePingResultV2>, this,
                std::placeholders::_1));
  ...
  reconfigure_server_->setCallback(
      boost::bind(&OculusDriver::configCallback, this, _1, _2));
}

// Updates sonar parameters
void OculusDriver::configCallback(
    const oculus_sonar_driver::OculusSonarConfig &config, uint32_t level) {
